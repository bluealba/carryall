...
class SlackReporter extends Reporter {
	constructor(config) {
		...
		this.flush = co.wrap(this.flush.bind(this));
	}
	...
	* flush(updateMessagePromise) {
		let updateMessage;
		if (updateMessagePromise) {
			updateMessage = yield updateMessagePromise;
		}
	...
	reportInstallStart() {
		this.clear();
		this.print(`${this.format.bold("Carryall")} ${emoji.get("construction")}  - starting`);
		this.startMessage = this.flush();
	}

	reportActionsDone(currentEnvironment, actionsDone, actionsNeeded) {
		this.clear();
		this.print(`${this.format.bold("Carryall")} ${emoji.get("truck")}  - finished`);
		this.reportEnvironment(currentEnvironment, actionsDone, actionsNeeded, false);
		this.flush(this.startMessage);
	}
	...
	reportError(error) {
		this.clear();
		this.print(`${this.format.bold("Carryall")} ${emoji.get("fire")}  - failure`);
		this.flush(this.startMessage);
	}
